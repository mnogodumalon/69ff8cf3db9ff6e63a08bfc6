...
interface HundeprofilDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: Hundeprofil['fields']) => Promise<void>;
  defaultValues?: Hundeprofil['fields'];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function HundeprofilDialog({ open, onClose, onSubmit, defaultValues, enablePhotoScan = true, enablePhotoLocation = true }: HundeprofilDialogProps) {
  const [fields, setFields] = useState<Partial<Hundeprofil['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'hundeprofil');
      await onSubmit(clean as Hundeprofil['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handleAiExtract(file?: File) {
    ...
    try {
      ...
      const schema = `{\n  "hund_name": string | null, // Name des Hundes\n  "rasse": string | null, // Rasse\n  "geburtsdatum": string | null, // YYYY-MM-DD\n  "geschlecht": LookupValue | null, // Geschlecht (select one key: "maennlich" | "weiblich") mapping: maennlich=Männlich, weiblich=Weiblich\n  "gewicht_kg": number | null, // Gewicht (kg)\n  "farbe": string | null, // Fellfarbe\n  "chip_nummer": string | null, // Chip-Nummer\n  "besitzer_vorname": string | null, // Vorname des Besitzers\n  "besitzer_nachname": string | null, // Nachname des Besitzers\n  "notizen_profil": string | null, // Anmerkungen\n}`;
      ...
      setFields(prev => {
        ...
        return merged as Partial<Hundeprofil['fields']>;
      });
  ...
  const DIALOG_INTENT = defaultValues ? 'Hundeprofil bearbeiten' : 'Hundeprofil hinzufügen';
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            <Label htmlFor="hund_name">Name des Hundes</Label>
            <Input
              id="hund_name"
          ...
          <div className="space-y-2">
            <Label htmlFor="chip_nummer">Chip-Nummer</Label>
            <Input
              id="chip_nummer"
              value={fields.chip_nummer ?? ''}
              onChange={e => setFields(f => ({ ...f, chip_nummer: e.target.value }))}
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="besitzer_vorname">Vorname des Besitzers</Label>
            <Input
              id="besitzer_vorname"
              value={fields.besitzer_vorname ?? ''}
              onChange={e => setFields(f => ({ ...f, besitzer_vorname: e.target.value }))}
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="besitzer_nachname">Nachname des Besitzers</Label>
            <Input
              id="besitzer_nachname"
              value={fields.besitzer_nachname ?? ''}
              onChange={e => setFields(f => ({ ...f, besitzer_nachname: e.target.value }))}
            />
          </div>
          <div className="space-y-2">
            <Label htmlFor="foto">Foto des Hundes</Label>
            {fields.foto ? (
          ...
          <div className="space-y-2">
            <Label htmlFor="notizen_profil">Anmerkungen</Label>
            <Textarea
              id="notizen_profil"
              value={fields.notizen_profil ?? ''}
              onChange={e => setFields(f => ({ ...f, notizen_profil: e.target.value }))}
              rows={3}
            />
          </div>
          <DialogFooter>
