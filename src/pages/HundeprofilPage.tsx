...
export default function HundeprofilPage() {
  const [records, setRecords] = useState<Hundeprofil[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Hundeprofil | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Hundeprofil | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Hundeprofil | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getHundeprofil());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Hundeprofil['fields']) {
    await LivingAppsService.createHundeprofilEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Hundeprofil['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateHundeprofilEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteHundeprofilEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      title="Hundeprofil"
      subtitle={`${records.length} Hundeprofil im System`}
      action={
    ...
    >
      <div className="relative w-full max-w-sm">
        ...
        <Input
          placeholder="Hundeprofil suchen..."
          value={search}
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('chip_nummer')}>
                <span className="inline-flex items-center gap-1">
                  Chip-Nummer
                  {sortKey === 'chip_nummer' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('besitzer_vorname')}>
                <span className="inline-flex items-center gap-1">
                  Vorname des Besitzers
                  {sortKey === 'besitzer_vorname' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('besitzer_nachname')}>
                <span className="inline-flex items-center gap-1">
                  Nachname des Besitzers
                  {sortKey === 'besitzer_nachname' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('notizen_profil')}>
                <span className="inline-flex items-center gap-1">
                  Anmerkungen
                  {sortKey === 'notizen_profil' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                <TableCell className="font-medium">{record.fields.hund_name ?? '—'}</TableCell>
                ...
                <TableCell>{record.fields.chip_nummer ?? '—'}</TableCell>
                <TableCell>{record.fields.besitzer_vorname ?? '—'}</TableCell>
                <TableCell>{record.fields.besitzer_nachname ?? '—'}</TableCell>
                ...
                <TableCell className="max-w-xs"><span className="truncate block">{record.fields.notizen_profil ?? '—'}</span></TableCell>
                <TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={12} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Hundeprofil. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
