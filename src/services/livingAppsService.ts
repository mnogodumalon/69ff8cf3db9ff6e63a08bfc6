...
import type { Hundeprofil, Aktivitaetserfassung, GesundheitFitness, CreateHundeprofil, CreateAktivitaetserfassung, CreateGesundheitFitness } from '@/types/app';
...
export class LivingAppsService {
  // --- HUNDEPROFIL ---
  static async getHundeprofil(): Promise<Hundeprofil[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.HUNDEPROFIL}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Hundeprofil[];
    return enrichLookupFields(records, 'hundeprofil');
  }
  static async getHundeprofilEntry(id: string): Promise<Hundeprofil | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.HUNDEPROFIL}/records/${id}`);
    const record = { record_id: data.id, ...data } as Hundeprofil;
    return enrichLookupFields([record], 'hundeprofil')[0];
  }
  static async createHundeprofilEntry(fields: CreateHundeprofil) {
    return callApi('POST', `/apps/${APP_IDS.HUNDEPROFIL}/records`, { fields: cleanFieldsForApi(fields as any, 'hundeprofil') });
  }
  static async updateHundeprofilEntry(id: string, fields: Partial<CreateHundeprofil>) {
    return callApi('PATCH', `/apps/${APP_IDS.HUNDEPROFIL}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'hundeprofil') });
  }
  static async deleteHundeprofilEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.HUNDEPROFIL}/records/${id}`);
  }

  // --- AKTIVITAETSERFASSUNG ---
  static async getAktivitaetserfassung(): Promise<Aktivitaetserfassung[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.AKTIVITAETSERFASSUNG}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as Aktivitaetserfassung[];
    return enrichLookupFields(records, 'aktivitaetserfassung');
  }
  static async getAktivitaetserfassungEntry(id: string): Promise<Aktivitaetserfassung | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.AKTIVITAETSERFASSUNG}/records/${id}`);
    const record = { record_id: data.id, ...data } as Aktivitaetserfassung;
    return enrichLookupFields([record], 'aktivitaetserfassung')[0];
  }
  static async createAktivitaetserfassungEntry(fields: CreateAktivitaetserfassung) {
    return callApi('POST', `/apps/${APP_IDS.AKTIVITAETSERFASSUNG}/records`, { fields: cleanFieldsForApi(fields as any, 'aktivitaetserfassung') });
  }
  static async updateAktivitaetserfassungEntry(id: string, fields: Partial<CreateAktivitaetserfassung>) {
    return callApi('PATCH', `/apps/${APP_IDS.AKTIVITAETSERFASSUNG}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'aktivitaetserfassung') });
  }
  static async deleteAktivitaetserfassungEntry(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.AKTIVITAETSERFASSUNG}/records/${id}`);
  }

  // --- GESUNDHEIT_&_FITNESS ---
  static async getGesundheitFitness(): Promise<GesundheitFitness[]> {
    const data = await callApi('GET', `/apps/${APP_IDS.GESUNDHEIT_FITNESS}/records`);
    const records = Object.entries(data).map(([id, rec]: [string, any]) => ({
      record_id: id, ...rec
    })) as GesundheitFitness[];
    return enrichLookupFields(records, 'gesundheit_&_fitness');
  }
  static async getGesundheitFitnes(id: string): Promise<GesundheitFitness | undefined> {
    const data = await callApi('GET', `/apps/${APP_IDS.GESUNDHEIT_FITNESS}/records/${id}`);
    const record = { record_id: data.id, ...data } as GesundheitFitness;
    return enrichLookupFields([record], 'gesundheit_&_fitness')[0];
  }
  static async createGesundheitFitnes(fields: CreateGesundheitFitness) {
    return callApi('POST', `/apps/${APP_IDS.GESUNDHEIT_FITNESS}/records`, { fields: cleanFieldsForApi(fields as any, 'gesundheit_&_fitness') });
  }
  static async updateGesundheitFitnes(id: string, fields: Partial<CreateGesundheitFitness>) {
    return callApi('PATCH', `/apps/${APP_IDS.GESUNDHEIT_FITNESS}/records/${id}`, { fields: cleanFieldsForApi(fields as any, 'gesundheit_&_fitness') });
  }
  static async deleteGesundheitFitnes(id: string) {
    return callApi('DELETE', `/apps/${APP_IDS.GESUNDHEIT_FITNESS}/records/${id}`);
  }
